...
describe('createStaticFilesMiddleware', () => {
  ...
  test('serves files with correct MIME types', async () => {
    ...
    for (const testCase of testCases) {
      ...
      const response = await middleware(request, {}, async () => new Response('Not found', { status: 404 }));
  ...
  test('handles URL encoding in file paths', async () => {
    ...
    const request = new Request('http://example.com/static/file%20with%20spaces.txt');
    const response = await middleware(request, {}, async () => new Response('Not found', { status: 404 }));
  ...
  test('handles concurrent requests to same file', async () => {
    ...
    const responses = await Promise.all(
      requests.map(request => 
        middleware(request, {}, async () => new Response('Not found', { status: 404 }))
      )
    );
