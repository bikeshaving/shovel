...
export async function createPlatform(platformName, options = {}) {
  switch (platformName) {
    ...
    default:
      throw new Error(`Unknown platform: ${platformName}. Supported platforms: node, bun, cloudflare`);
  }
}
...
export function displayPlatformInfo(platformName) {
  ...
  console.log(`[Shovel] Platform: ${platformName} ${isExplicit ? '(explicit)' : '(detected)'}`);
