...
describe('Router.mount()', () => {
  test('mounts subrouter at specified path', async () => {
    ...
    subrouter.route('/users/:id').get(async (request, context) => new Response(`User ${context.params.id}`));
  ...
  test('preserves route parameters in mounted subrouters', async () => {
    const subrouter = new Router();
    subrouter.route('/posts/:postId/comments/:commentId').get(
      async (request, context) => new Response(`Post ${context.params.postId}, Comment ${context.params.commentId}`)
    );
    
    const mainRouter = new Router();
    mainRouter.mount('/api/v1', subrouter);
    
    const request = new Request('http://example.com/api/v1/posts/456/comments/789');
  ...
  test('mounts subrouter middleware globally', async () => {
    ...
    const subrouterMiddleware = async (request: Request, context: any, next: () => Promise<Response>) => {
    ...
    expect(executionOrder).toEqual(['subrouter-middleware', 'subrouter-handler']);
  ...
  test('supports multiple HTTP methods on mounted routes', async () => {
    const subrouter = new Router();
    subrouter.route('/resource/:id')
      .get(async (request, context) => new Response(`GET ${context.params.id}`))
      .post(async (request, context) => new Response(`POST ${context.params.id}`))
      .put(async (request, context) => new Response(`PUT ${context.params.id}`))
      .delete(async (request, context) => new Response(`DELETE ${context.params.id}`));
    ...
    for (const { method, expected } of testCases) {
      const request = new Request('http://example.com/api/resource/123', { method });
      const response = await mainRouter.match(request);
  ...
  test('allows mounting multiple subrouters', async () => {
    ...
    usersRouter.route('/:id').get(async (request, context) => new Response(`User ${context.params.id}`));
    
    const postsRouter = new Router();
    postsRouter.route('/').get(async () => new Response('Posts List'));
    postsRouter.route('/:id').get(async (request, context) => new Response(`Post ${context.params.id}`));
  ...
  test('supports nested mounting (mounting routers with mounted subrouters)', async () => {
    ...
    commentsRouter.route('/:commentId').get(
      async (request, context) => new Response(`Comment ${context.params.commentId}`)
    );
    
    // Create middle level router and mount comments
    const postsRouter = new Router();
    postsRouter.route('/:postId').get(
      async (request, context) => new Response(`Post ${context.params.postId}`)
    );
    ...
    // Test nested route
    const commentRequest = new Request('http://example.com/api/posts/123/comments/456');
  ...
  test('preserves cache configuration from mounted subrouters', async () => {
    ...
    const subrouter = new Router();
    subrouter.route({ pattern: '/cached', cache: { name: 'test-cache' } })
