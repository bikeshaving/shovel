...
export function validateCacheConfig(config: CacheBackendConfig): void {
  ...
  if (!validTypes.includes(config.type)) {
    throw new Error(`Invalid cache type '${config.type}'. Must be one of: ${validTypes.join(', ')}`);
  }
 ...
 */
export function mergeHeaders(...headerSources: (Headers | Record<string, string> | undefined)[]): Headers {
 ...
 */
export function createPreflightResponse(corsConfig: any, request: Request): Response {
