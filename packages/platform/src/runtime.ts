...
/**
 * Generic worker interface for supervisor coordination.
 * Platforms provide implementations wrapping their native Worker APIs.
 */
export interface SupervisorWorker {
	...
	/** Optional close handler (for crash detection) */
	onclose?: ((event: {code?: number}) => void) | null;
}

/**
 * Options for initializing the supervisor runtime
 */
export interface InitSupervisorRuntimeOptions {
	/** Number of workers to spawn */
	workerCount: number;
	/** Factory to create a worker instance */
	createWorker: () => SupervisorWorker;
	/** Optional: called when all workers are ready */
	onReady?: () => void;
	/** Optional: called on worker error */
	onWorkerError?: (error: Error) => void;
	/** Optional: called when a worker crashes */
	onWorkerCrash?: (exitCode: number) => void;
}

/**
 * Result from initializing the supervisor runtime
 */
export interface InitSupervisorRuntimeResult {
	/** Handler for HTTP requests (dispatches to workers via round-robin) */
	handleRequest: (request: Request) => Promise<Response>;
	/** Gracefully shut down all workers */
	shutdown: () => Promise<void>;
	/** Whether all workers are ready */
...
/**
 * Initialize the supervisor runtime for production multi-worker deployments.
 *
 * The supervisor:
 * - Spawns the specified number of workers
 * - Dispatches requests to workers via round-robin
 ...
 * ```
 */
export function initSupervisorRuntime(
	options: InitSupervisorRuntimeOptions,
): InitSupervisorRuntimeResult {
	const {workerCount, createWorker, onReady, onWorkerError, onWorkerCrash} =
		options;
	const logger = getLogger(["shovel", "platform"]);

	const workers: SupervisorWorker[] = [];
	let currentWorker = 0;
	let requestId = 0;
	...
	for (let i = 0; i < workerCount; i++) {
		...
		worker.onmessage = (event) => {
			...
			if (data.type === "ready") {
				...
				if (readyCount === workerCount) {
					logger.info("All workers ready", {workers: workerCount});
					onReady?.();
					readyResolve?.();
				}
			} else if (data.type === "response") {
				// WorkerResponseMessage format
				const msg = event.data as WorkerResponseMessage;
				...
				// WorkerErrorMessage format
				const msg = event.data as WorkerErrorMessage;
			...
			} else if (data.type === "shutdown-complete") {
		...
		worker.onerror = (event) => {
			...
			onWorkerError?.(event.error || new Error("Unknown worker error"));
		};

		// Handle worker crashes (if supported by the worker implementation)
		if ("onclose" in worker) {
			(
				worker as SupervisorWorker & {onclose: (event: {code?: number}) => void}
			).onclose = (event) => {
				if (shuttingDown) return;
				if (event.code === 0) return;
				logger.error("Worker crashed", {exitCode: event.code});
				onWorkerCrash?.(event.code ?? 1);
			};
		}
	...
	/**
	 * Dispatch a request to a worker and wait for response
	 */
	async function handleRequest(request: Request): Promise<Response> {
		...
		// Send request to worker using WorkerRequestMessage format
	...
	async function shutdown(): Promise<void> {
		...
		// Wait for all workers to acknowledge shutdown (with timeout)
		const timeout = 10000;
		const start = Date.now();
		while (workers.length > 0 && Date.now() - start < timeout) {
