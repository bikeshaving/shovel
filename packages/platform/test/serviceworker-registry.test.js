...
test(
	...
	async () => {
		const { ServiceWorkerContainer } = await import("../src/service-worker-api.js");
...
test(
	...
	async () => {
		...
		const adminReg = await container.register('/admin-worker.js', { scope: '/admin/' });
...
test(
	...
	async () => {
		...
		await expect(container.handleRequest(apiRequest)).rejects.toThrow("No response provided for fetch event");
		await expect(container.handleRequest(adminRequest)).rejects.toThrow("No response provided for fetch event");
		await expect(container.handleRequest(rootRequest)).rejects.toThrow("No response provided for fetch event");
	},
	TIMEOUT
);
