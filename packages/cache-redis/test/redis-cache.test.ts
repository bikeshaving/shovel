...
describe("RedisCache", () => {
	...
	describe("match", () => {
		test("should return undefined when no cached response exists", async () => {
			...
			expect(mockRedisClient.get).toHaveBeenCalledWith("cache:test:GET:http://example.com/api");
		});
	...
	describe("delete", () => {
		test("should delete cached entry", async () => {
			...
			expect(mockRedisClient.del).toHaveBeenCalledWith("cache:test:GET:http://example.com/api");
		});
