...
class CloudflareKVCache extends MemoryCache {
  ...
  async put(request: Request, response: Response): Promise<void> {
    ...
    await this.kv.put(key, value, {
      expirationTtl: this.options.ttl ? Math.floor(this.options.ttl / 1000) : undefined
    });
  }
...
export class CloudflarePlatform implements Platform {
  ...
  createCaches(config: CacheConfig = {}): CacheStorage {
    ...
    if (this.options.kvNamespaces) {
      for (const [name, kv] of Object.entries(this.options.kvNamespaces)) {
        caches.register(name, () => new CloudflareKVCache(name, kv, config[name]));
      }
    }
  ...
  async loadServiceWorker(entrypoint: string, options: ServiceWorkerOptions = {}): Promise<ServiceWorkerInstance> {
    ...
    const isCloudflareWorker = typeof globalThis.addEventListener === 'function' &&
 ...
 */
export function createCloudflarePlatform(options?: CloudflarePlatformOptions): CloudflarePlatform {
