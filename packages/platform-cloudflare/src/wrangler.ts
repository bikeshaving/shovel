...
export function generateWranglerConfig(options: {
  ...
  const { name, entrypoint, kvNamespaces = [], r2Buckets = [], d1Databases = [] } = options;
...
# KV bindings
${kvNamespaces.map(kv => `[[kv_namespaces]]
binding = "${kv}"
id = "your-kv-namespace-id"
preview_id = "your-preview-kv-namespace-id"`).join('\n\n')}

# R2 bindings  
${r2Buckets.map(bucket => `[[r2_buckets]]
binding = "${bucket}"
bucket_name = "your-bucket-name"`).join('\n\n')}

# D1 bindings
${d1Databases.map(db => `[[d1_databases]]
binding = "${db}"
database_name = "your-database-name"
database_id = "your-database-id"`).join('\n\n')}
`;
}