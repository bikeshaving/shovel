...
const pattern1 = new URLPattern({ pathname: '/api/posts', search: 'type=blog&sort=date' });
...
const url1c = new URL('http://example.com/api/posts?type=blog&sort=date&extra=value'); // Extra param

console.log('Pattern:', pattern1.search);
console.log('URL 1a (exact match):', url1a.search, '→', pattern1.test(url1a));
console.log('URL 1b (different order):', url1b.search, '→', pattern1.test(url1b));
...
console.log('URL 4c (with page & limit):', url4c.search, '→', pattern4.test(url4c));
...
console.log('1. Search params must match exactly (order matters, no extra params)');
