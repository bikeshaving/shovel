...
describe('MatchPattern vs URLPattern', () => {
  describe('Search Parameter Matching', () => {
    test('URLPattern fails with different parameter order', () => {
      const pattern: URLPattern = new URLPattern({ pathname: '/api/posts', search: 'type=blog&sort=date' });
      const url1: URL = new URL('http://example.com/api/posts?type=blog&sort=date');
      const url2: URL = new URL('http://example.com/api/posts?sort=date&type=blog');
    ...
    test('URLPattern fails with extra parameters', () => {
      const pattern: URLPattern = new URLPattern({ pathname: '/api/posts', search: 'type=blog&sort=date' });
      const url: URL = new URL('http://example.com/api/posts?type=blog&sort=date&extra=value');
    ...
    test('MatchPattern handles different parameter order', () => {
      const pattern: MatchPattern = new MatchPattern({ pathname: '/api/posts', search: 'type=:type&sort=:sort' });
      const url1: URL = new URL('http://example.com/api/posts?type=blog&sort=date');
      const url2: URL = new URL('http://example.com/api/posts?sort=date&type=blog');
  ...
  describe('String Parameter Parsing', () => {
    ...
    test('MatchPattern should support full URL strings', () => {
      const pattern: MatchPattern = new MatchPattern('https://api.example.com/v1/posts/:id');
  ...
  describe('Parameter Merging', () => {
    test('URLPattern keeps pathname and search params separate', () => {
      const pattern: URLPattern = new URLPattern({ pathname: '/api/:version/posts/:id', search: 'format=:format' });
      const url: URL = new URL('http://example.com/api/v1/posts/123?format=json');
    ...
    test('MatchPattern should merge all parameters', () => {
      const pattern: MatchPattern = new MatchPattern({ pathname: '/api/:version/posts/:id', search: 'format=:format' });
      const url: URL = new URL('http://example.com/api/v1/posts/123?format=json&page=1');
      const result: MatchPatternResult | null = pattern.exec(url);
  ...
  describe('Enhanced Routing Features', () => {
    test('MatchPattern should support non-exhaustive search matching', () => {
      const pattern: MatchPattern = new MatchPattern({ pathname: '/search', search: 'q=:query' });
      const url1: URL = new URL('http://example.com/search?q=hello');
      const url2: URL = new URL('http://example.com/search?q=hello&page=1');
      const url3: URL = new URL('http://example.com/search?q=hello&page=1&limit=10');
    ...
    test('MatchPattern should handle order-independent search params', () => {
      const pattern: MatchPattern = new MatchPattern({ pathname: '/test', search: 'type=:type&sort=:sort' });
...
describe('MatchPattern API Design', () => {
  ...
  test('should support convenience pattern formats', () => {
    ...
    const pattern2: MatchPattern = new MatchPattern('/api/posts/:id&format=:format');
  ...
  test('should match URLs with & syntax patterns', () => {
    const pattern: MatchPattern = new MatchPattern('/api/posts/:id&format=:format');
