...
describe("Trailing Slash Normalization", () => {
	...
	describe("MatchPattern trailing slash normalization", () => {
		...
		test("should preserve parameters with trailing slash normalization", () => {
			...
			const result1: MatchPatternResult | null = pattern.exec(
				new URL("http://example.com/api/posts/123"),
			);
			const result2: MatchPatternResult | null = pattern.exec(
				new URL("http://example.com/api/posts/123/"),
			);
		...
		test("should not normalize trailing slashes in the middle of paths", () => {
			...
			expect(pattern.test(new URL("http://example.com/api/posts/123"))).toBe(
				false,
			);
		});
