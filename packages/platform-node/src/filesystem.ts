...
export class NodeFileSystemFileHandle implements FileSystemFileHandle {
	...
	async createSyncAccessHandle(): Promise<FileSystemSyncAccessHandle> {
		throw new DOMException('Synchronous access handles are only available in workers', 'InvalidStateError');
	}
 ...
 */
export class NodeFileSystemDirectoryHandle implements FileSystemDirectoryHandle {
	...
	async getFileHandle(name: string, options?: { create?: boolean }): Promise<FileSystemFileHandle> {
		...
		try {
			...
			if (!stats.isFile()) {
				throw new DOMException('Path exists but is not a file', 'TypeMismatchError');
			}
		} catch (error) {
			if ((error as any).code === 'ENOENT') {
				if (options?.create) {
	...
	async getDirectoryHandle(name: string, options?: { create?: boolean }): Promise<FileSystemDirectoryHandle> {
		...
		try {
			...
			if (!stats.isDirectory()) {
				throw new DOMException('Path exists but is not a directory', 'TypeMismatchError');
			}
		} catch (error) {
			if ((error as any).code === 'ENOENT') {
				if (options?.create) {
